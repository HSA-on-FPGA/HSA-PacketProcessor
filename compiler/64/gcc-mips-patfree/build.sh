...
tar xf newlib-2.2.0.20150623.tar.gz
echo " done!"
...
patch ./binutils-2.25/opcodes/mips-opc.c < ./binutils-patch/mips-opc.c.patch
echo " done!"
echo -n "Building binutils..."
mkdir build-binutils
cd build-binutils
../binutils-2.25/configure --target=mips64el-elf --prefix=$PREFIX --disable-nls > /dev/null
make -j 9 > /dev/null
...
mkdir build-gmp
cd build-gmp
../gmp-6.0.0/configure --prefix=$PREFIX --disable-shared > /dev/null
make -j 9 > /dev/null
make install > /dev/null
cd ..
echo " done!"
...
mkdir build-mpfr
cd build-mpfr
../mpfr-3.1.3/configure --prefix=$PREFIX --disable-shared --with-gmp=$PREFIX > /dev/null
make -j 9 > /dev/null
make install > /dev/null
cd ..
echo " done!"
...
rm newlib-2.2.0.20150623/newlib/libc/machine/mips/memcpy.S
cp ./newlib-patches/Makefile.am newlib-2.2.0.20150623/newlib/libc/machine/mips/
cp ./newlib-patches/Makefile.in newlib-2.2.0.20150623/newlib/libc/machine/mips/

# build newlib
echo -n "Building newlib..."
oldCFLAGS=$CFLAGS
export CFLAGS_FOR_TARGET="-mno-unaligned-mem-access -DSMALL_MEMORY -Os -mips3 -mabi=64 -mlong64 -T ../linker.ld -mno-gpopt -mno-branch-likely"
mkdir build-newlib
cd build-newlib
../newlib-2.2.0.20150623/configure --target=mips64el-elf --prefix=$PREFIX > /dev/null
...
export SYSROOT_CFLAGS_FOR_TARGET="-mno-unaligned-mem-access -Os -mips3 -mabi=64 -mlong64 -T ../linker.ld -mno-gpopt -mno-branch-likely -mnohwdiv -mnohwmult -mno-gpopt"
